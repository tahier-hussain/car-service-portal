  ...
  submitHandler = event => {
    ...
    axios(requestOptions).then(res => {
      if (res.data.status === 200) {
        ...
        requestOptions = {
          method: "POST",
          url: "http://localhost:5000/api/service-centre-login",
          headers: {
            "Content-Type": "application/json"
          },
          data: {
        ...
        axios(requestOptions).then(serviceCentre => {
          if (serviceCentre.data.status === 200) {
            localStorage.setItem("user-details", JSON.stringify(serviceCentre.data.users));
            localStorage.setItem("auth-token", serviceCentre.data.token);
            this.props.history.push("/home");
          } else if (serviceCentre.data.status === 400) {
            this.setState({
              error_message: serviceCentre.data.msg
            });
          }
        });
      }
